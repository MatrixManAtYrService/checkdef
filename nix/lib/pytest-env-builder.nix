...
in
{
  meta = {
    ...
    makesChanges = false;
    isDerivedCheck = true;
    description = "Creates pytest derivations with environment builder function - eliminates redundant filtering";
    ...
    envBuilder = {
      ...
      note = "This function is called once per unique filter pattern, enabling efficient caching and eliminating redundant filtering";
    };
    includePatterns = {
      description = "Glob patterns for files to include in the test derivation for cache isolation";
      example = [ "src/mymodule/**" "tests/test_mymodule.py" ];
  ...
  pattern =
    { src
    , envBuilder
    , name ? "pytest-env-builder"
    , description ? "Python tests with env builder"
    , testConfig ? { }
    ...
    let
      ...
      # Single source filtering using simple glob pattern matching
      ...
      # Apply filtering to create cache-isolated source - SINGLE FILTERING STEP
      filteredSrc = filterSource includePatterns src;

      # Build Python environment using the provided builder function
      # This is where uv2nix, poetry, or other tools would be called
