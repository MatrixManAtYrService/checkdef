        ...
        builtins.attrValues (builtins.mapAttrs
          (checkName: derivation:
            ...
            ''
                ...
                # In non-verbose mode, allow stderr to flow but don't show verbose output
                # The key issue is that 2>/dev/null can cause hangs with interactive prompts
                if nix build "''$target_derivation" --out-link "''$result_link"; then
                  build_success=true
                else
                  build_success=false
                fi
                  ...
                  test_execution_time=''$(grep -E '[0-9]+ passed.*in [0-9.]+s' "''$result_link/build_logs.txt" 2>/dev/null | grep -o '[0-9.]\+s' | head -1)
                                fi

                                # Simple timing display - just show the test time if available, otherwise build time
