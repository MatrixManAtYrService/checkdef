#!/usr/bin/env python3
"""
Universal check runner framework for development projects.

This runner provides beautiful output with Rich formatting, cache detection,
and comprehensive error reporting. It's designed to be reusable across
any project that uses Nix-based checks.
"""
...
app = typer.Typer(help="Universal check runner with rich output and caching awareness")
console = Console()


class CheckResult:
    def __init__(self, name: str, path: str, success: bool, cached: bool, output: str = ""):
        self.name = name
        self.path = path
...
def display_summary(results: List[CheckResult], suite_name: str):
    ...
    stats_table.add_row(
        "Passed:",
        f"[green]{len(passed)}[/green] | Failed: [red]{len(failed)}[/red]",
    )
    stats_table.add_row(
        "Executed:",
    ...
    if failed:
        console.print(
            "[red]Some checks failed. Please review the output above.[/red]"
        )
        sys.exit(1)
    else:
...
def main(
    ...
    suite_name: str = typer.Option("Checks", "--suite-name", help="Name of the check suite"),
